...
(function () {
  ...
  function lane(i) {
    ...
    function poll(n, myGen) {
      ...
      if (t && !emitted) {
        emitted = true;
        log('PHASE lane=' + i + ' gen=' + myGen +
            ' inter=' + Math.round(tInter) + ' rect=' + Math.round(tRect) +
            ' clickat=' + Math.round(tClick) + ' tok=' + Math.round(now() - tBuild) +
            ' polls=' + ticks + ' nclickat=' + nClickAt);
      ...
      if (n * POLL_MS > 600000) { defer(myGen, 0); return; }
    ...
    function recycle(myGen) {
      if (myGen !== gen) return;
      // A generation that dies WITHOUT a token is the expensive one; it is
      // invisible in any token-keyed metric, so report it explicitly.
      if (!emitted) {
        log('PHASEX lane=' + i + ' gen=' + myGen +
            ' inter=' + Math.round(tInter) + ' rect=' + Math.round(tRect) +
            ' clickat=' + Math.round(tClick) + ' age=' + Math.round(now() - tBuild) +
            ' polls=' + ticks + ' nclickat=' + nClickAt);
      }
      try { window.turnstile.remove(id); } catch (e) {}
      id = null;
      setTimeout(build, RECYCLE_MS);
    }
...
RE_PHASE = re.compile(
    r"^PHASE lane=(\d+) gen=(\d+) inter=(-?\d+) rect=(-?\d+) "
    r"clickat=(-?\d+) tok=(-?\d+) polls=(\d+) nclickat=(\d+)")
RE_PHASEX = re.compile(
    r"^PHASEX lane=(\d+) gen=(\d+) inter=(-?\d+) rect=(-?\d+) "
    r"clickat=(-?\d+) age=(-?\d+) polls=(\d+) nclickat=(\d+)")
...
class Stats:
    def __init__(self) -> None:
        ...
        # ── PROBE accumulators ──────────────────────────────────────────
        self.phase: list[dict] = []       # generations that produced a token
        self.phasex: list[dict] = []      # generations that died empty
        self.clicklat: list[dict] = []    # one row per dispatched click
        self.click_inflight = 0           # clicks currently holding the pointer
        self.click_overlap = 0            # dispatches that began while >0 in flight
        self.click_inflight_max = 0
...
async def run_session(args, stats: Stats, out_handle, deadline: float | None,
    ...
    pointer_lock = threading.Lock()   # the X pointer, when --click-serialize
    inflight_lock = threading.Lock()  # guards the in-flight counters only

    async with open_page(args, ladder) as page:
        await page.route("**/*", _block_irrelevant)

        def do_click(lane: int, gen: int, px: int, py: int,
                     queued_at: float = 0.0) -> None:
            """Deliver one click, fully instrumented.

            THE X POINTER IS ONE GLOBAL RESOURCE. `run_in_executor(None, ...)`
            hands this function to the default thread pool, which on a 4-vCPU
            runner has 8 workers — so up to 8 lanes can be inside
            `xdotool mousemove/mousedown/mouseup` at once, on ONE pointer.
            When they interleave, lane A's mousedown fires wherever lane B
            last moved the pointer: a click that is dispatched, counted and
            logged as successful, and lands nowhere near lane A's widget.
            `inflight` is the direct measure of how often that happens.
            """
            key = (lane, gen)
            n = clicks_at.get(key, 0)
            driver = "xdo"
            t_start = time.monotonic()
            row = {"lane": lane, "gen": gen, "n": n,
                   "q_ms": round((t_start - queued_at) * 1000, 1)}
            if xdo.available and n < args.click_retry:
                t_lock = time.monotonic()
                ctx = pointer_lock if args.click_serialize else contextlib.nullcontext()
                with ctx:
                    row["lock_ms"] = round((time.monotonic() - t_lock) * 1000, 1)
                    with inflight_lock:
                        if stats.click_inflight > 0:
                            stats.click_overlap += 1
                        row["inflight"] = stats.click_inflight
                        stats.click_inflight += 1
                        stats.click_inflight_max = max(stats.click_inflight_max,
                                                       stats.click_inflight)
                    try:
                        t_geo = time.monotonic()
                        screen = xdo.to_screen(px, py, geom)
                        row["geo_ms"] = round((time.monotonic() - t_geo) * 1000, 1)
                        ok = False
                        if screen:
                ...
                if ok:
                    ...
                    stats.clicks_xdo += 1
                    row["driver"] = "xdo"
                    row["total_ms"] = round((time.monotonic() - t_start) * 1000, 1)
                    stats.clicklat.append(row)
                    print("CLICKLAT " + json.dumps(row, sort_keys=True), flush=True)
                    return
        ...
        def on_console(message):
            ...
            if match:
                ...
                now = time.monotonic()
                stats.clickat_seen += 1
                if now - last_click.get(lane, 0.0) < args.click_interval:
                    stats.clickat_dropped_interval += 1
                    return
                if clicks_at.get((lane, gen), 0) >= args.click_max:
                    stats.clickat_dropped_max += 1
                    return
                last_click[lane] = now
                loop.run_in_executor(None, do_click, lane, gen, px, py, now)
                return

            match = RE_PHASE.match(text)
            if match:
                stats.phase.append({
                    "lane": int(match.group(1)), "gen": int(match.group(2)),
                    "inter": int(match.group(3)), "rect": int(match.group(4)),
                    "clickat": int(match.group(5)), "tok": int(match.group(6)),
                    "polls": int(match.group(7)), "nclickat": int(match.group(8)),
                    "wall": round(time.monotonic() - stats.started, 1),
                })
                print("  " + text, flush=True)
                return

            match = RE_PHASEX.match(text)
            if match:
                stats.phasex.append({
                    "lane": int(match.group(1)), "gen": int(match.group(2)),
                    "inter": int(match.group(3)), "rect": int(match.group(4)),
                    "clickat": int(match.group(5)), "age": int(match.group(6)),
                    "polls": int(match.group(7)), "nclickat": int(match.group(8)),
                })
                print("  " + text, flush=True)
                return
...
async def main() -> int:
    ...
    parser.add_argument("--poll-ms", type=int, default=1000,
                        help="widget poll tick. The production value is 1000, "
                             ...
                             "token and ~0.5 s detecting the rect — ~1 s of "
                             "every cycle, spent waiting on our own timer.")
    ...
    parser.add_argument("--click-serialize", action="store_true",
                        help="hold a global lock across the whole xdotool "
                             ...
                             "pointer; without this, concurrent lane clicks "
    ...
    def spread(rows: list[dict], field: str, keep=lambda r: True) -> dict:
        vals = [r[field] for r in rows if field in r and keep(r)]
        return {"n": len(vals), "p50": pct(vals, 0.5), "p90": pct(vals, 0.9),
                "p99": pct(vals, 0.99),
                "mean": round(sum(vals) / len(vals), 1) if vals else -1.0}

    print("PROBE-PHASE " + json.dumps({
        "tokens_with_phase": len(stats.phase),
        "dead_generations": len(stats.phasex),
        # ms from build() to each milestone, for generations that DID mint.
        "inter_ms": spread(stats.phase, "inter", lambda r: r["inter"] >= 0),
        "rect_ms": spread(stats.phase, "rect", lambda r: r["rect"] >= 0),
        "tok_ms": spread(stats.phase, "tok"),
        "polls": spread(stats.phase, "polls"),
        "clickat_lines": spread(stats.phase, "nclickat"),
        "dead_age_ms": spread(stats.phasex, "age"),
        "dead_escalated": sum(1 for r in stats.phasex if r["inter"] >= 0),
        "dead_rect_ok": sum(1 for r in stats.phasex if r["rect"] >= 0),
    }, sort_keys=True), flush=True)

    xdo_rows = [r for r in stats.clicklat if r.get("driver") == "xdo"]
    print("PROBE-CLICK " + json.dumps({
        "dispatched": len(stats.clicklat),
        "clickat_seen": stats.clickat_seen,
        "clickat_dropped_interval": stats.clickat_dropped_interval,
        "clickat_dropped_max": stats.clickat_dropped_max,
        "overlapped": stats.click_overlap,
        "overlap_pct": round(100.0 * stats.click_overlap / len(stats.clicklat), 1)
                       if stats.clicklat else -1.0,
        "inflight_max": stats.click_inflight_max,
        "inflight_hist": dict(sorted(collections.Counter(
            r["inflight"] for r in stats.clicklat if "inflight" in r).items())),
        "queue_ms": spread(stats.clicklat, "q_ms"),
