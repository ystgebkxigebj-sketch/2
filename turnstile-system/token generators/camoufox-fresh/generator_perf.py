...
# survives — but acceptance is re-verified per arm here anyway, because a rate
# gain that costs acceptance is worthless.
LEAN_PREFS = {
...
(function () {
  ...
  // ── CYCLE PACING KNOBS (perf variant only) ────────────────────────────────
  // POLL_MS is the interval at which a lane asks turnstile.getResponse() and
  // re-measures its rect. It is NOT free latency: a token that Cloudflare
  // finished 10 ms after a tick is not seen for another POLL_MS, and a widget
  // that just escalated does not emit CLICKAT for up to another POLL_MS. At the
  // stock 1000 ms that is up to 2 s of pure self-inflicted delay per token on a
  // ~12 s cycle. REBUILD_MS is the pause between turnstile.remove() and the next
  // render(); it was a flat 400 ms.
  var POLL_MS = __POLL_MS__;
  var REBUILD_MS = __REBUILD_MS__;
  // Keep the give-up backstop at ~10 minutes of WALL CLOCK, not 600 ticks:
  // scaling POLL_MS down without this silently turns the backstop into a
  // 3-minute killer that destroys challenges Cloudflare was still working on.
  var MAX_TICKS = Math.ceil(600000 / POLL_MS);
  ...
  function lane(i) {
    ...
    function build() {
      ...
      // PHASE MARKER. Everything the cycle breakdown measures is anchored here:
      // BUILD -> RECT(open) -> INTER -> CLICKAT -> [click] -> CBACK -> TOK, and
      // then TOK -> the NEXT BUILD of the same lane is our own reset+rebuild
      // pause. Without this line the reset delay is indistinguishable from
      // Cloudflare being slow, which is the whole question.
      log('BUILD lane=' + i + ' gen=' + myGen);
      ...
      try {
        id = window.turnstile.render(slot, {
          ...
          callback: function () {
            // The token is still READ via getResponse() (this callback has been
            // observed firing EMPTY), but its FIRING TIME is the only way to
            // separate "Cloudflare was slow" from "our 1 Hz poll had not looked
            // yet": CBACK -> TOK is pure polling latency and nothing else.
            log('CBACK lane=' + i + ' gen=' + myGen);
          },
    ...
    function poll(n, myGen) {
      ...
      if (n > MAX_TICKS) { defer(myGen, 0); return; }
      setTimeout(function () { poll(n + 1, myGen); }, POLL_MS);
    }
...
def build_renderer(args) -> str:
    return (
        ...
        .replace("__REBUILD_MS__", str(int(args.rebuild_ms)))
...
RE_BUILD = re.compile(r"^BUILD lane=(\d+) gen=(\d+)")
RE_CBACK = re.compile(r"^CBACK lane=(\d+) gen=(\d+)")
RE_INTER = re.compile(r"^INTER lane=(\d+) gen=(\d+)")


# ---------------------------------------------------------------------------
# THE CYCLE BREAKDOWN — the instrument this whole variant exists for
#
# tokens/min is one over the mean cycle time, and a cycle is a SUM of phases we
# control and phases Cloudflare controls. Optimising before you know which is
# which is guesswork: on 2026-07-31 a GitHub runner measured 15.6% of 4 cores at
# lanes=8 while throughput FELL, so freeing CPU on this host cannot be the lever
# — but nobody had ever measured where the ~12 s per token actually went.
#
# Phases, all anchored on the (lane, gen) pair so concurrent lanes never mix:
#
#   build   -> rect_open   widget laid out at a clickable size
#   build   -> inter       Cloudflare decided to escalate to interactive
#   rect_open -> clickat   OUR poll granularity before the click is requested
#   clickat -> click       click_interval gating + xdotool pointer motion
#   click   -> cback       Cloudflare's actual solve time after the click
#   cback   -> tok         OUR poll granularity before the token is collected
#   tok     -> next build  OUR reset delay (token_interval) + rebuild pause
#
# The last three plus rect_open->clickat are OURS. If they dominate, a rate gain
# is available for free; if click->cback dominates, the wall is Cloudflare and no
# amount of pacing will help. Reported as medians, because a couple of 300 s
# timeouts would drag any mean into nonsense.
# ---------------------------------------------------------------------------
CYCLE_PHASES = (
    "build_to_rect", "build_to_inter", "inter_to_clickat", "clickat_to_click",
    "click_to_cback", "cback_to_tok", "build_to_tok", "tok_to_next_build",
    "full_cycle",
)


class CycleTracker:
    """Per-(lane, gen) phase timestamps, reduced to medians on demand."""

    def __init__(self) -> None:
        # Bounded: a 5-hour run builds thousands of generations and an unbounded
        # dict here would be a slow leak inside a process that is already the
        # memory ceiling on a 16 GB runner.
        self.marks: collections.OrderedDict[tuple[int, int], dict[str, float]] = (
            collections.OrderedDict()
        )
        self.samples: dict[str, collections.deque[float]] = {
            phase: collections.deque(maxlen=4000) for phase in CYCLE_PHASES
        }
        self.last_tok_at: dict[int, float] = {}       # lane -> monotonic of TOK
        self.last_build_at: dict[int, float] = {}     # lane -> monotonic of BUILD

    def _slot(self, lane: int, gen: int) -> dict[str, float]:
        key = (lane, gen)
        slot = self.marks.get(key)
        if slot is None:
            slot = {}
            self.marks[key] = slot
            while len(self.marks) > 400:
                self.marks.popitem(last=False)
        return slot

    def add(self, phase: str, seconds: float) -> None:
        if 0.0 <= seconds < 900.0:
            self.samples[phase].append(seconds)

    def build(self, lane: int, gen: int, now: float) -> None:
        self._slot(lane, gen)["build"] = now
        previous_tok = self.last_tok_at.pop(lane, None)
        if previous_tok is not None:
            # OUR pause: token_interval + the rebuild delay + JS timer slop.
            self.add("tok_to_next_build", now - previous_tok)
        previous_build = self.last_build_at.get(lane)
        if previous_build is not None:
            self.add("full_cycle", now - previous_build)
        self.last_build_at[lane] = now

    def mark(self, lane: int, gen: int, phase: str, now: float, once: bool = True) -> None:
        slot = self._slot(lane, gen)
        if once and phase in slot:
            return
        slot[phase] = now

    def token(self, lane: int, gen: int, now: float) -> None:
        slot = self._slot(lane, gen)
        slot["tok"] = now
        self.last_tok_at[lane] = now
        # NOTE ON inter_to_clickat, because the obvious phase is the wrong one.
        # RECT and CLICKAT are emitted from the SAME poll iteration, so
        # rect->clickat is structurally ~0 and measures nothing. The real
        # self-inflicted delay is INTER (Cloudflare's own escalation callback,
        # which fires the instant it decides) -> CLICKAT (the next poll tick that
        # notices the rect). That interval is bounded above by POLL_MS and is
        # exactly the latency a faster poll would buy back.
        pairs = (("build", "rect_open", "build_to_rect"),
                 ("build", "inter", "build_to_inter"),
                 ("inter", "clickat", "inter_to_clickat"),
                 ("clickat", "click", "clickat_to_click"),
                 ("click", "cback", "click_to_cback"),
                 ("cback", "tok", "cback_to_tok"),
                 ("build", "tok", "build_to_tok"))
        for start, end, phase in pairs:
            if start in slot and end in slot:
                self.add(phase, slot[end] - slot[start])
    ...
    def summary(self) -> dict:
        ...
        for phase in CYCLE_PHASES:
            ...
            out[phase] = {"n": len(values), "p50": round(at(0.5), 2),
                          "p90": round(at(0.9), 2)}
        return out
...
class Stats:
    def __init__(self) -> None:
        ...
        self.diverted = 0
        # Lives on Stats so the phase histogram survives the browser restarts
        # that end each session, exactly like the token counters do.
...
async def open_page(args, ladder: "Ladder | None"):
    ...
    if args.geoip:
        launch["geoip"] = True
    if args.no_ubo:
        # uBlock Origin is Camoufox's ONLY default addon. On the Oracle VM it
        # measured ~11% of a core with nothing to block — every non-document,
        # non-Cloudflare request is already aborted by _block_irrelevant. On a
        # GitHub runner CPU is not the constraint (15.6% of 4 cores at lanes=8),
        # so the reason to test it here is LATENCY: uBO still filters every
        # request the challenge makes, and cycle latency is what sets tok/min.
...
async def run_session(args, stats: Stats, out_handle, deadline: float | None,
    ...
    async with open_page(args, ladder) as page:
        ...
        def do_click(lane: int, gen: int, px: int, py: int) -> None:
            ...
            if xdo.available and n < args.click_retry:
                ...
                if screen:
                    ok = xdo.click(*screen)
                    if ok:
                        # Marked AFTER delivery, not at dispatch: the humanised
                        # pointer motion is ~0.5 s of OUR time and belongs in
                        # clickat_to_click, not in Cloudflare's solve.
                        stats.cycle.mark(lane, gen, "click", time.monotonic())
                        clicks_at[key] = n + 1
                        stats.clicks += 1
                        stats.clicks_xdo += 1
        ...
        def on_console(message):
            ...
            # ── phase markers first: they are the highest-frequency lines and
            # they must be timestamped as close to arrival as possible.
            match = RE_BUILD.match(text)
            if match:
                stats.cycle.build(int(match.group(1)), int(match.group(2)),
                                  time.monotonic())
                return

            match = RE_CBACK.match(text)
            if match:
                stats.cycle.mark(int(match.group(1)), int(match.group(2)),
                                 "cback", time.monotonic())
                return
            ...
            if match:
                ...
                mono = time.monotonic()
                stats.cycle.token(lane, gen, mono)
            ...
            if match:
                ...
                now = time.monotonic()
                stats.cycle.mark(lane, gen, "clickat", now)
            ...
            if match:
                lane, gen = int(match.group(1)), int(match.group(2))
                w, h = int(match.group(3)), int(match.group(4))
                if w >= args.click_min_w and h >= args.click_min_h:
                    stats.cycle.mark(lane, gen, "rect_open", time.monotonic())
            ...
            match = RE_INTER.match(text)
            if match:
                stats.cycle.mark(int(match.group(1)), int(match.group(2)),
                                 "inter", time.monotonic())
                stats.escalations += 1
                print(f"  {text}", flush=True)
                return
        ...
        # Measured, not assumed — and printed once per browser so a run that
        # restarts cannot silently change what is loaded halfway through.
        print(f"  [addons] no_ubo_flag={args.no_ubo} lean_prefs={args.lean_prefs} "
              f"| {probe_addons()}", flush=True)
        ...
        while True:
            ...
            if now - last_diag >= args.diag_every:
                ...
                print("CYCLE " + json.dumps(stats.cycle.summary(), sort_keys=True),
                      flush=True)
...
async def main() -> int:
    ...
    # ── PERF ARMS. Every one of these defaults to the value the 10.26 tok/min
    # baseline ran at, so an arm that forgets a flag is a BASELINE arm and not a
    # silently half-applied one.
    parser.add_argument("--no-ubo", action="store_true",
                        help="drop Camoufox's uBlock Origin addon (its only "
                             "default addon). Cost ~11%% of a core on the VM; "
                             "here the hypothesis is request latency, not CPU.")
    ...
    parser.add_argument("--poll-ms", type=float, default=1000,
                        help="in-page poll interval. 1000 is the stock value "
                             "and adds up to 1 s before a finished token is "
                             "collected plus up to 1 s before an escalated "
                             "widget is clicked.")
    parser.add_argument("--rebuild-ms", type=float, default=400,
                        help="pause between turnstile.remove() and the next "
                             "render(). 400 is the stock value.")
    ...
    print(
        "PRODUCER " + json.dumps({
            ...
            # The arm's identity travels WITH its numbers. Five concurrent runs
            # produce five logs that are otherwise indistinguishable, and an arm
            # whose flag silently failed to apply must not be readable as a
            # result for that flag.
    ...
    print("CYCLE " + json.dumps(stats.cycle.summary(), sort_keys=True), flush=True)
